...
public sealed partial class RadarApp : IDisposable
{
    ...
    private readonly PerformanceCadence _atlasRouteCadence = new();
    ...
    private Dictionary<nint, int> _atlasRouteHopsCache = new();
    ...
    private NumVec2? _atlasCurrentPt;
    ...
    private void Tick()
    {
        ...
        var ctx = new RenderContext(
            ...
            AtlasDefaultBackgroundColor: _settings.AtlasDefaultBackgroundColor,
            AtlasDefaultFontColor: _settings.AtlasDefaultFontColor,
        ...
        return new
        {
            ...
            nodeList = nodes
                ...
                .Select(n => new
                {
                    ...
                    unlocked = n.Unlocked, visited = n.Visited, completed = n.Completed, accessible = n.AccessibleNow,
                    visible = n.Visible, contentCount = n.ContentCount,
                    x = (int)n.X, y = (int)n.Y, sx = (int)n.ScreenCenter.X, sy = (int)n.ScreenCenter.Y,
                    code = n.MapCode, map = n.MapName, tags = n.Tags, badges = n.Badges,
                }),
        };
    }
    ...
    private long ComputeAtlasContentSig(int nodeCount)
    {
        ...
        unchecked
        {
            ...
            h = (h * 31) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(_settings.AtlasSearchQuery ?? "");
            h = (h * 31) ^ (_settings.AtlasHideCompletedMaps ? 1 : 0);
            h = (h * 31) ^ (_settings.AtlasHideNotAccessibleMaps ? 1 : 0);
            h = (h * 31) ^ (_settings.AtlasHideAvailableMaps ? 1 : 0);
            h = (h * 31) ^ (_settings.AtlasRouteGroups.Sum(g => (g.DrawPaths ? 1 : 0) + g.Entries.Count(e => e.DrawPath)));
            return h;
        }
    }
    ...
    private void UpdateAtlas(nint inGameState)
    {
        ...
        var needContentChips = _settings.AtlasShowContentTokens || _settings.AtlasShowContentIcons
            || _settings.AtlasShowContentBadges || _settings.AtlasShowContentCount;
        ...
        foreach (var n in nodes)
        {
            ...
            if (needContentChips && n.Tags is { Count: > 0 })
            {
                AtlasCatalog.Shared.CategorizeContentTags(n.Tags, out var flagInfos, out var contentInfos);
                if (flagInfos.Count > 0) flagChips = flagInfos.Select(AtlasContentChip.FromInfo).ToArray();
                if (contentInfos.Count > 0) contentChips = contentInfos.Select(AtlasContentChip.FromInfo).ToArray();
    ...
    private readonly record struct AtlasRouteTarget(Poe2Atlas.AtlasNodeLive Node, string Label, string Color, int MaxHops, float LineThickness = 1f);

    private readonly record struct AtlasRouteGeometry(Dictionary<nint, int> HopsByElement, IReadOnlyList<AtlasRouteLine> Lines);

    private AtlasRouteGeometry BuildAtlasRoutes(
        IReadOnlyList<Poe2Atlas.AtlasNodeLive> nodes,
        IReadOnlyList<AtlasRouteTarget> targets)
    {
        var hopsMap = new Dictionary<nint, int>();
        if (targets.Count == 0) return new AtlasRouteGeometry(hopsMap, Array.Empty<AtlasRouteLine>());
        ...
        foreach (var t in targets)
        {
            ...
            var thickness = t.LineThickness > 0 ? t.LineThickness : _settings.AtlasRouteLineThickness;
            lines.AddRange(AtlasRoutePolylineBuilder.BuildSegments(path, gridToScreen, t.Label, t.Color, hops,
                thickness, phase++));
        }

        var routeLines = lines.Count > 0 ? lines.Take(256).ToArray() : Array.Empty<AtlasRouteLine>();
    ...
    private static bool AtlasRouteEntryMatches(in Poe2Atlas.AtlasNodeLive node, string match)
    {
        ...
        var kind = parts.Length == 2 ? parts[0] : "map";
        var value = parts.Length == 2 ? parts[1] : match;
    ...
    private void BuildAtlasRoute(IReadOnlyList<Poe2Atlas.AtlasNodeLive> nodes, IReadOnlyList<AtlasRouteTarget> routeTargets)
    {
        ...
        _atlasCurrentPt = null;
        if (_atlas.CurrentMarkerCenter(OverlayWidth, OverlayHeight) is { } markerCenter)
            _atlasCurrentPt = new NumVec2(markerCenter.X, markerCenter.Y);
        else if (_atlas.CurrentNodeGrid() is { } cur && gridToScreen.TryGetValue(cur, out var cp))
            _atlasCurrentPt = cp;
        ...
        if (_atlasGoalGrid is { } goal)
        {
            ...
            if (path != null)
            {
                var pts = new List<NumVec2>(path.Count);
                var allResolved = true;
                foreach (var p in path)
                {
                    if (gridToScreen.TryGetValue(p, out var rp)) pts.Add(rp);
                    else { allResolved = false; break; }
                }
                if (allResolved && pts.Count >= 2) _atlasRoute = pts;
            }
