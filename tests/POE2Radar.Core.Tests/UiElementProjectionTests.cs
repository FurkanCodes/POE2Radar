...
{
    [Fact]
    public void ScalePair_PreservesEstablishedOverlayScaleIndexMapping()
    {
    ...
    [Fact]
    public void DefaultProjection_PreservesFullClientWidth_OnUltrawideClient()
    {
        const uint visible = 1u << Poe2.UiElement.FlagVisibleBit;
        var element = new UiElementProjection.Element(
            1, 0, visible, 100, 50, 0, 0, 1f, 0, 40, 40);
        ...
        Assert.True(ok);
        Assert.Equal(134.375f, rect.X, 3);
        Assert.Equal(67.1875f, rect.Y, 3);
        Assert.Equal(53.75f, rect.W, 3);

        bool ReadElement(nint address, out UiElementProjection.Element projected)
        {
            projected = element;
            return address == 1;
        }
